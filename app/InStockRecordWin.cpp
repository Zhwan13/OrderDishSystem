...
/******************************
*函数名 InStockRecordWin()
*函数功能：窗体界面构造函数，对窗体的控件进行初始化
*输入 窗体的坐标和宽度、高度、窗体名字
*输出 无
************************************/
InStockRecordWin::InStockRecordWin(int x,int y,int height,int width,char *text,void *data,int winNo)
:CWinBase(x,y,height,width,text,winNo)
{
    ...
    this->Start=new CLabel(this->x+3,this->y+6,2,6,"开始:");
    this->End=new CLabel(this->x+3,this->y+9,2,6,"结束:");
    this->StartYear=new CEdit(this->x+11,this->y+6,2,10,"",EDIT);
    this->StartMon=new CEdit(this->x+26,this->y+6,2,8,"",EDIT);
    this->StartDay=new CEdit(this->x+39,this->y+6,2,8,"",EDIT);
    this->EndYear=new CEdit(this->x+11,this->y+9,2,10,"",EDIT);
    this->EndMon=new CEdit(this->x+26,this->y+9,2,8,"",EDIT);
    this->EndDay=new CEdit(this->x+39,this->y+9,2,8,"",EDIT);

    this->SearchbyDateBtn=new CButton(this->x+(this->width)-16,this->y+6,2,14,"按入库时间查询");
    ...
    this->InStockDateLab=new CLabel(this->x+31,this->y+12,2,6,"入库日期");
    this->InStockNumLab=new CLabel(this->x+43,this->y+12,2,6,"入库数量");
    this->LocationNoLab=new CLabel(this->x+55,this->y+12,2,6,"仓位编号");
    this->InStockRecTab=new CTable(this->x+6,this->y+11,8,60,this->data,RECORDTABLE);//10,5,22,80,RECORDTABLE
    ...
    this->addControl(this->InStockRecTab);
    ...
    this->addControl(this->InStockDateLab);
    this->addControl(this->InStockNumLab);
...
InStockRecordWin::~InStockRecordWin()
{
...
*返回 界面号
************************************/
int InStockRecordWin::doAction(void *data)
{
    int flag=0;//判断是否查找到
    ...
    this->InStockRecTab->setData(data);//先刷一下表格的数据，以免在没有找到后打印乱码
    if(strcmp(this->controls[this->curControl]->getText(),"按入库时间查询")==0)
    {
        ...
        {//判断输入框是否都输有数据
            ...
            if(flag==1)
                this->InStockRecTab->setData(Recordfind);
            else if(flag==0)
            {
                CTool::gotoxy(20,30);
                cout<<"no find";
        ...
        return INSTOCKRECORDWIN;//返回入库记录界面
    }
    else if(strcmp(this->controls[this->curControl]->getText(),"返回")==0)
    {
        return INSTOCKMENUWIN;
    }
    return INSTOCKRECORDWIN;//防止程序崩溃
}
...
*输出 无
************************************/
void InStockRecordWin::UpdateControls()
{
